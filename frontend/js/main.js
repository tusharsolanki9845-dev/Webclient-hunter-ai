...
const SearchPage = {
  ...
  renderDiscovery(payload) {
    ...
    prospects.forEach(prospect => {
      ...
      const note = tag('div', 'discovery-note', 'Public website mapped in the source. Run source-labelled checks and review the homepage before deciding whether outreach is appropriate.'); const details = tag('div', 'discovery-details'); card.append(note, details);
      const actions = tag('div', 'lead-actions'); const enrich = action('Check website & public contact', 'btn btn-secondary btn-sm', () => this.enrichDiscoveryLead(prospect, enrich, details)); const audit = tag('a', 'btn btn-primary btn-sm', 'Open website checks'); audit.href = `reports.html?url=${encodeURIComponent(prospect.url)}&name=${encodeURIComponent(prospect.name)}`;
  ...
  async enrichDiscoveryLead(prospect, button, details) {
    setBusy(button, true, 'Checking website…');
    try {
      const route = `/discovery${Auth.demo() ? '/demo' : ''}/enrich`; const result = await API.post(route, { url: prospect.url });
      if (!result.ok) return Toast.error('Website check could not run', result.error);
      ...
      const rating = tag('div', 'rating-result'); if (report?.scores) { rating.append(tag('span', `badge ${report.scores.overall < 40 ? 'badge-red' : report.scores.overall < 60 ? 'badge-yellow' : 'badge-green'}`, `${report.scores.overall}/100 heuristic check`)); const scoreText = tag('span', '', `SEO ${report.scores.seo} · Response ${report.scores.speed} · Mobile markup ${report.scores.mobile}`); rating.appendChild(scoreText); }
      ...
      button.disabled = true; button.textContent = 'Checks loaded'; Toast.success('Website checks loaded', prospect.emails.length ? 'A generic public email was observed on the fetched homepage.' : 'No generic public email was observed on the fetched homepage.');
...
function scores(seo, speed, mobile) {
  const values = { seo, speed, mobile };
  [['score-seo', seo], ['score-speed', speed], ['score-mobile', mobile]].forEach(([id, value]) => { const node = $(id); if (!node) return; node.textContent = value; const circle = node.closest('.score-circle'); if (circle) circle.className = `score-circle ${scoreClass(value)}`; });
  [['seo-label', seo], ['speed-label', speed], ['mobile-label', mobile]].forEach(([id, value]) => text($(id), Number.isFinite(value) ? `${value}/100` : '—'));
  document.querySelectorAll('.progress-fill[data-key]').forEach(node => { const value = values[node.dataset.key]; if (Number.isFinite(value)) { node.dataset.score = value; node.style.width = `${value}%`; node.style.background = scoreColor(value); } });
}
...
function setPageSpeedScore(id, value) { const node = $(id); if (!node) return; node.textContent = Number.isFinite(value) ? value : '—'; const circle = node.closest('.score-circle'); if (circle && Number.isFinite(value)) circle.className = `score-circle ${scoreClass(value)}`; }
function renderPageSpeedReport(pageSpeed) {
  $('pagespeed-panel')?.removeAttribute('hidden');
  setPageSpeedScore('pagespeed-performance', pageSpeed.categories?.performance);
  setPageSpeedScore('pagespeed-accessibility', pageSpeed.categories?.accessibility);
  setPageSpeedScore('pagespeed-seo', pageSpeed.categories?.seo);
  text($('pagespeed-status'), `${pageSpeed.source || 'Google PageSpeed Insights'} · ${pageSpeed.strategy || 'mobile'} lab report · ${new Date(pageSpeed.analyzedAt || Date.now()).toLocaleString()}`);
  const link = $('pagespeed-source-link'); if (link) { link.href = pageSpeed.sourceUrl || '#'; link.hidden = !pageSpeed.sourceUrl; }
  const metrics = pageSpeed.metrics || {}; const metricText = [['FCP', metrics.firstContentfulPaint], ['LCP', metrics.largestContentfulPaint], ['TBT', metrics.totalBlockingTime], ['CLS', metrics.cumulativeLayoutShift]].filter(([, value]) => value).map(([label, value]) => `${label}: ${value}`).join(' · ');
...
async function initReports() {
  ...
  $('audit-form')?.addEventListener('submit', async event => { event.preventDefault(); const url = $('audit-url-input')?.value.trim(); if (!url) return Toast.warning('Enter a website URL.'); const button = $('run-audit-btn'); setBusy(button, true, 'Checking…'); try { if (Auth.demo()) report = demoReport(url); else { const result = await API.post('/audit', { url, ...(selectedLead ? { leadId: selectedLead.id } : {}) }); if (!result.ok) return Toast.error('Heuristic checks could not run', result.error); report = result.data?.data; } document.querySelectorAll('.audit-business-name').forEach(node => text(node, selectedLead?.name || report.url)); document.querySelectorAll('.audit-url').forEach(node => text(node, report.url)); text($('audit-source-label'), Auth.demo() ? 'Explicit demo: sample heuristic checks' : 'Heuristic website checks'); text($('audit-date'), `Checked ${new Date(report.auditedAt || Date.now()).toLocaleString()}`); scores(report.scores.seo, report.scores.speed, report.scores.mobile); renderIssues(report.issues || []); Toast.success('Heuristic checks complete.'); } finally { setBusy(button, false); } });
  $('run-pagespeed-btn')?.addEventListener('click', async () => { const url = $('audit-url-input')?.value.trim(); if (!url) return Toast.warning('Enter a website URL first.'); if (Auth.demo()) return Toast.info('PageSpeed is available after signing in', 'The sample preview does not call external data services.'); const button = $('run-pagespeed-btn'); setBusy(button, true, 'Requesting PageSpeed…'); try { const result = await API.post('/audit/pagespeed', { url, ...(selectedLead ? { leadId: selectedLead.id } : {}) }); if (!result.ok) return Toast.error('PageSpeed report could not run', result.error); renderPageSpeedReport(result.data?.data || {}); Toast.success('PageSpeed report loaded.'); } finally { setBusy(button, false); } });
}
